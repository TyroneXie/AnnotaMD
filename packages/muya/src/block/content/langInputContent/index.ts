...
import { withCodeInfoLanguage } from '../../../utils/codeInfoString';
...
class LangInputContent extends Content {
    ...
    constructor(muya: Muya, { meta }: ICodeBlockState) {
        ...
        this.attributes.hint = muya.i18n.t('Code Block');
        this.attributes.contenteditable = 'false';
        this.createDomNode();
        this.muya.eventCenter.attachDOMEvent(this.domNode!, 'click', (event) => {
            event.preventDefault();
            event.stopPropagation();
            this.parent?.lastContentInDescendant()?.setCursor(0, 0, true);
        });
    }
    ...
    private _updateInfoString(info: string, source?: 'code-caption') {
        ...
        if (cursor) {
            const startOffset = Math.min(info.length, cursor.start.offset);
            const endOffset = Math.min(info.length, cursor.end.offset);
    ...
    private _updateLanguage(lang: string) {
        this._updateInfoString(withCodeInfoLanguage(this.text, lang));
    }
