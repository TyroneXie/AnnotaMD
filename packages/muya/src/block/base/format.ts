...
type InlineStyleFormat = keyof typeof INLINE_STYLE_PROPERTIES;

function isInlineStyleFormat(type: string): type is InlineStyleFormat {
    return type in INLINE_STYLE_PROPERTIES;
}

function normalizeInlineColor(value?: string): string {
    const color = value?.trim().toLowerCase() ?? '';
    return /^#[\da-f]{6}$/.test(color) ? color : '';
}

export function isInlineStyleFormatToken(token: Token, type?: InlineStyleFormat): boolean {
    if (token.type !== 'html_tag' || token.tag.toLowerCase() !== 'span')
        return false;

    const style = token.attrs.style ?? '';
    const properties = type ? [INLINE_STYLE_PROPERTIES[type]] : Object.values(INLINE_STYLE_PROPERTIES);
    return properties.some(property => new RegExp(`(?:^|;)\\s*${property}\\s*:`).test(style));
}

function tokenMatchesFormat(token: Token, type: string): boolean {
    if (isInlineStyleFormat(type))
        return isInlineStyleFormatToken(token, type);

    return token.type === type || (token.type === 'html_tag' && token.tag === type);
}
...
function getOffset(offset: number, token: Token) {
    ...
    switch (type) {
        ...
        case 'html_tag': {
            const openLength = token.openTag?.length ?? FORMAT_TAG_MAP[token.tag]?.open.length ?? 0;
            const closeLength = token.closeTag?.length ?? FORMAT_TAG_MAP[token.tag]?.close.length ?? 0;
...
function checkTokenIsInlineFormat(token: Token) {
    ...
    if (type === 'html_tag')
        return /^(?:u|sub|sup|mark)$/i.test(token.tag) || isInlineStyleFormatToken(token);
...
class Format extends Content {
    ...
    override backspaceHandler(event: Event): void {
        ...
        // fix: #897 in pre-migration implementation repo
        const { text } = this;
    ...
    private _scanBackspaceTokens(tokens: Token[], offset: number): {
        ...
        for (const token of tokens) {
            ...
            // Fix: https://github.com/TyroneXie/AnnotaMD/issues/113
    ...
    format(type: string, value?: string) {
        ...
        const [currentFormats, currentNeighbors] = [formats, neighbors].map(
            item =>
                item
                    .filter(format => tokenMatchesFormat(format, type))
                    .reverse(),
        );

        const inlineColor = isInlineStyleFormat(type) ? normalizeInlineColor(value) : '';
        ...
        if (type === 'clear') {
            ...
            if (inlineColor)
                this._addFormat(type, { start, end }, inlineColor);
        }
        else {
            ...
            if (!isInlineStyleFormat(type) || inlineColor)
                this._addFormat(type, { start, end }, inlineColor);
    ...
    private _addFormat(
        ...
        switch (type) {
            ...
            case 'background_color': {
                const property = INLINE_STYLE_PROPERTIES[type];
