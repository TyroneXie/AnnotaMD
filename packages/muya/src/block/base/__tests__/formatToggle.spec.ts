...
describe('format.format() persistent inline colors', () => {
    it('stores font color as inline HTML', () => {
        const content = selectInFirstBlock(bootMuya('abc\n'), 0, 3);

        content.format('text_color', '#3370ff');

        expect(content.text).toBe('<span style="color: #3370ff">abc</span>');
    });

    it('stores background color as inline HTML', () => {
        const content = selectInFirstBlock(bootMuya('abc\n'), 0, 3);

        content.format('background_color', '#fff2b8');

        expect(content.text).toBe('<span style="background-color: #fff2b8">abc</span>');
    });

    it('replaces an existing font color instead of nesting another color span', () => {
        const markdown = '<span style="color: #f54a45">abc</span>\n';
        const start = markdown.indexOf('abc');
        const content = selectInFirstBlock(bootMuya(markdown), start, start + 3);
    ...
    it('restores the default font color by removing the matching span', () => {
        const markdown = '<span style="color: #3370ff">abc</span>\n';
        const start = markdown.indexOf('abc');
        const content = selectInFirstBlock(bootMuya(markdown), start, start + 3);

        content.format('text_color', '');

        expect(content.text).toBe('abc');
    });

    it('round-trips the inline style through markdown serialization and reload', async () => {
        const first = bootMuya('abc\n');
        selectInFirstBlock(first, 0, 3).format('background_color', '#d9f7be');

        await vi.waitFor(() => {
            expect(first.getMarkdown()).toContain('<span style="background-color: #d9f7be">abc</span>');
        });

        const reopened = bootMuya(first.getMarkdown());
        const styled = reopened.domNode.querySelector<HTMLElement>('span[style*="background-color"]');
        expect(styled).not.toBeNull();
        expect(styled!.style.backgroundColor).toBe('#d9f7be');
    });
});
