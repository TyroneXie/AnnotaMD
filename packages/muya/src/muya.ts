...
export class Muya {
    ...
    setOptions(options: Partial<IMuyaOptions>, forceRender = false) {
        ...
        if ('wrapCodeBlocks' in options) {
            const label = options.wrapCodeBlocks
                ? this.i18n.t('Cancel Wrap')
                : this.i18n.t('Auto Wrap');
            this.domNode.querySelectorAll<HTMLElement>('.mu-code-wrap-toggle').forEach((button) => {
                button.title = label;
                const labelNode = button.querySelector<HTMLElement>('.mu-code-action-label');
                if (labelNode)
