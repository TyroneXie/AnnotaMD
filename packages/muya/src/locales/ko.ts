export const ko = {
    name: 'ko',
    resource: {
    // tableTools
        'Insert Row Above': '위에 행 삽입',
        'Insert Row Below': '아래에 행 삽입',
        'Remove Row': '행 삭제',
        // tableColumnTools
        'Align Left': '왼쪽 정렬',
        'Align Center': '가운데 정렬',
        'Align Right': '오른쪽 정렬',
        'Insert Column left': '왼쪽에 열 삽입',
        'Insert Column right': '오른쪽에 열 삽입',
        'Remove Column': '열 삭제',
        'Insert Column Left': '왼쪽에 열 삽입',
        'Insert Column Right': '오른쪽에 열 삽입',
        'Move Row Up': '행 위로 이동',
        'Move Row Down': '행 아래로 이동',
        'Move Column Left': '열 왼쪽 이동',
        'Move Column Right': '열 오른쪽 이동',
        'Reset Column Width': '열 너비 재설정',
        // quickInsert
        'Paragraph': '단락',
        'Horizontal Line': '수평선',
        'Front Matter': '머리말 블록',
        'Heading 1': '제목 1',
        'Heading 2': '제목 2',
        'Heading 3': '제목 3',
        'Heading 4': '제목 4',
        'Heading 5': '제목 5',
        'Heading 6': '제목 6',
        'Set Number': '번호 설정',
        'Continue Heading Numbering': '제목 번호 이어가기',
        'Restart Numbering': '번호 다시 시작',
        'Set Number Value': '번호 값 설정',
        'Number Value': '번호 값',
        'Cancel': '취소',
        'Table Block': '표',
        'Display Math': '수식',
        'HTML Block': 'HTML 블록',
        'Code Block': '코드 블록',
        'Quote Block': '인용',
        'Highlight Block': '강조 블록',
        'Emoji': 'Emoji',
        'Order List': '번호 목록',
        'Bullet List': '글머리 기호 목록',
        'To-do List': '할 일 목록',
        'Vega Chart': 'Vega 차트',
        'Mermaid': 'Mermaid',
        'Plantuml': 'Plantuml',
        'basic blocks': '기본 블록',
        'headings': '제목',
        'advanced blocks': '고급 블록',
        'list blocks': '목록',
        'diagrams': '다이어그램',
        'No result': '결과 없음',
        'Font Color': '글자 색',
        'Background Color': '배경 색',
        'Text and Background Color': '글자 및 배경 색',
        'Default Color': '기본 색',
        'Copy': '복사',
        'Wrap': '줄 바꿈',
        'Wrap lines': '자동 줄 바꿈',
        'Auto Wrap': '자동 줄 바꿈',
        'Cancel Wrap': '줄 바꿈 해제',
        'Change Language': '언어 변경',
        'Search Languages...': '언어 검색...',
        'Plain Text': '일반 텍스트',
        'More Languages': '더 많은 언어',
        'Fewer Languages': '언어 접기',
        'Search keyword...': '키워드 검색...',
        'Type / to insert...': '/ 입력하여 삽입...',
        'Copy anchor link to this heading': '이 제목의 앵커 링크 복사',
        'Click to add an image': '클릭하여 이미지 추가',
        'Load image failed': '이미지 로드 실패',
        // formatPicker
        'Emphasize': '굵게',
        'Italic': '기울임',
        'Underline': '밑줄',
        'Strikethrough': '취소선',
        'Highlight': '강조',
        'Inline Code': '인라인 코드',
        'Inline Math': '인라인 수식',
        'Link': '링크',
        'Edit Link': '링크 편집',
        'Text': '텍스트',
        'Link Address': '링크',
        'Paste or enter link': '링크 붙여넣기 또는 입력',
        'Confirm': '확인',
        'Remove Link': '링크 제거',
        'Link View': '링크 보기',
        'Title View': '제목 보기',
        'Copy Link': '링크 복사',
        'Copy Original Web Link': '원본 웹페이지 링크 복사',
        'More': '더보기',
        'Image': '이미지',
        'Eliminate': '서식 지우기',
        // Code block
        'Copy content': '내용 복사',
        'Input Language Identifier...': '언어 식별자 입력...',
        'Diagram View': '보기',
        'Code and Chart': '코드와 차트 표시',
        'Code Only': '코드만 표시',
        'Chart Only': '차트만 표시',
        'Fullscreen': '전체 화면',
        'Diagram Background': '차트 배경색',
        'Copy Diagram': '차트 복사',
        'Download Diagram': '차트 다운로드',
        // emojiPicker
        'Smileys & Emotion': '스마일 & 감정',
        'People & Body': '사람 & 신체',
        'Animals & Nature': '동물 & 자연',
        'Food & Drink': '음식 & 음료',
        'Travel & Places': '여행 & 장소',
        'Activities': '활동',
        'Objects': '사물',
        'Symbols': '기호',
        'Flags': '국기',
        // frontMenu
        'Copy Plain Text': '일반 텍스트 복사',
        'Copy Markdown': 'Markdown 복사',
        'Copy Section': '섹션 복사',
        'Duplicate Section': '섹션 복제',
        'Cut Section': '섹션 잘라내기',
        'Promote Section Headings': '섹션 제목 수준 올리기',
        'Demote Section Headings': '섹션 제목 수준 내리기',
        'Collapse Section': '섹션 접기',
        'Expand Section': '섹션 펼치기',
        'Move Section Up': '섹션 위로 이동',
        'Move Section Down': '섹션 아래로 이동',
        'Delete Section': '섹션 삭제',
        'Confirm Delete Section': '다시 클릭하여 삭제',
        'Delete Section Confirmation': '이 제목과 섹션의 모든 내용을 삭제하시겠습니까?',
        'Duplicate': '단락 복제',
        'Cut Block': '블록 잘라내기',
        'New Paragraph': '새 단락',
        'Insert Above': '위에 삽입',
        'Insert Below': '아래에 삽입',
        'Move Up': '위로 이동',
        'Move Down': '아래로 이동',
        'Comment': '댓글',
        'Delete Selection': '선택 영역 삭제',
        'Delete': '단락 삭제',
        // imageToolbar
        'Edit Image': '이미지 편집',
        'Edit Block': 'Edit Block',
        'Delete Block': 'Delete Block',
        'Inline Image': '인라인 이미지',
        'Remove Image': '이미지 삭제',
        // ImageSelector
        'Select': '선택',
        'Embed link': '링크 삽입',
        'Choose Image': '이미지 선택',
        'Choose image from your computer.': '컴퓨터에서 이미지를 선택하세요.',
        'Alt text': '대체 텍스트',
        'Image link or local path': '이미지 링크 또는 로컬 경로',
        'Image title': '이미지 제목',
        'Embed Image': '이미지 삽입',
        'Paste web image or local image path. Use': '웹 이미지 또는 로컬 경로를 붙여넣기. 모드 전환',
        'simple mode': '간단 모드',
        'full mode': '전체 모드',
        // preview block
        'Loading...': '로딩 중...',
        'Invalid Diagram Code': '잘못된 다이어그램 코드',
        'Empty Diagram': '빈 다이어그램',
        'Input Mathematical Formula...': '수식 입력...',
        'Input Front Matter...': '머리말 입력...',
        'Invalid Mathematical Formula': '잘못된 수식',
        'Empty Mathematical Formula': '빈 수식',
    },
};
