export const zhTW = {
    name: 'zh-TW',
    resource: {
    // tableTools
        'Insert Row Above': '在上方插入列',
        'Insert Row Below': '在下方插入列',
        'Remove Row': '刪除所在列',
        // tableColumnTools
        'Align Left': '靠左對齊',
        'Align Center': '置中對齊',
        'Align Right': '靠右對齊',
        'Insert Column left': '在左邊插入欄',
        'Insert Column right': '在右邊插入欄',
        'Remove Column': '刪除所在欄',
        'Insert Column Left': '在左側插入欄',
        'Insert Column Right': '在右側插入欄',
        'Move Row Up': '上移列',
        'Move Row Down': '下移列',
        'Move Column Left': '左移欄',
        'Move Column Right': '右移欄',
        'Reset Column Width': '恢復預設欄寬',
        // quickInsert
        'Paragraph': '一般段落',
        'Horizontal Line': '水平分隔線',
        'Front Matter': '頂部資訊區塊',
        'Heading 1': '標題 1',
        'Heading 2': '標題 2',
        'Heading 3': '標題 3',
        'Heading 4': '標題 4',
        'Heading 5': '標題 5',
        'Heading 6': '標題 6',
        'Set Number': '設定編號',
        'Continue Heading Numbering': '繼續標題編號',
        'Restart Numbering': '重新開始編號',
        'Set Number Value': '設定編號的值',
        'Number Value': '編號值',
        'Cancel': '取消',
        'Table Block': '表格',
        'Display Math': '數學公式',
        'HTML Block': 'HTML 區塊',
        'Code Block': '程式碼區塊',
        'Quote Block': '引言',
        'Highlight Block': '高亮區塊',
        'Emoji': 'Emoji',
        'Order List': '有序清單',
        'Bullet List': '無序清單',
        'To-do List': '待辦清單',
        'Vega Chart': 'Vega 圖',
        'Mermaid': 'Mermaid',
        'Plantuml': 'Plantuml',
        'basic blocks': '基礎區塊',
        'headings': '標題',
        'advanced blocks': '進階區塊',
        'list blocks': '清單',
        'diagrams': '圖表',
        'No result': '無結果',
        'Font Color': '文字顏色',
        'Background Color': '背景顏色',
        'Text and Background Color': '文字與背景顏色',
        'Default Color': '預設顏色',
        'Copy': '複製',
        'Wrap': '換行',
        'Wrap lines': '自動換行',
        'Auto Wrap': '自動換行',
        'Cancel Wrap': '取消換行',
        'Change Language': '更換程式語言',
        'Search Languages...': '搜尋程式語言...',
        'Plain Text': '純文字',
        'More Languages': '更多語言',
        'Fewer Languages': '收起更多語言',
        'Search keyword...': '搜尋關鍵字...',
        'Type / to insert...': '輸入 / 插入段落',
        'Copy anchor link to this heading': '複製此標題的錨點連結',
        'Click to add an image': '點擊新增圖片',
        'Load image failed': '圖片載入失敗',
        // formatPicker
        'Emphasize': '粗體',
        'Italic': '斜體',
        'Underline': '底線',
        'Strikethrough': '刪除線',
        'Highlight': '醒目提示',
        'Inline Code': '行內程式碼',
        'Inline Math': '行內數學公式',
        'Link': '超連結',
        'Edit Link': '編輯連結',
        'Text': '文字',
        'Link Address': '連結',
        'Paste or enter link': '貼上或輸入連結',
        'Confirm': '確認',
        'Remove Link': '取消連結',
        'Link View': '連結檢視',
        'Title View': '標題檢視',
        'Copy Link': '複製連結',
        'Copy Original Web Link': '複製原始網頁連結',
        'More': '更多',
        'Image': '圖片',
        'Eliminate': '清除樣式',
        // Code block
        'Copy content': '複製內容',
        'Input Language Identifier...': '輸入程式語言識別碼...',
        'Diagram View': '檢視',
        'Code and Chart': '顯示程式碼和圖表',
        'Code Only': '僅顯示程式碼',
        'Chart Only': '僅顯示圖表',
        'Fullscreen': '全螢幕',
        'Diagram Background': '圖表背景色',
        'Copy Diagram': '複製圖表',
        'Download Diagram': '下載圖表',
        // emojiPicker
        'Smileys & Emotion': '笑臉 & 情緒',
        'People & Body': '人物 & 身體',
        'Animals & Nature': '動物 & 自然',
        'Food & Drink': '食物 & 飲料',
        'Travel & Places': '旅遊 & 地點',
        'Activities': '活動',
        'Objects': '物件',
        'Symbols': '符號',
        'Flags': '旗幟',
        // frontMenu
        'Copy Plain Text': '複製純文字',
        'Copy Markdown': '複製 Markdown',
        'Copy Section': '複製章節內容',
        'Duplicate Section': '建立章節副本',
        'Cut Section': '剪下本章節',
        'Promote Section Headings': '整節提升標題層級',
        'Demote Section Headings': '整節降低標題層級',
        'Collapse Section': '摺疊本章節',
        'Expand Section': '展開本章節',
        'Move Section Up': '整節上移',
        'Move Section Down': '整節下移',
        'Delete Section': '刪除本章節',
        'Confirm Delete Section': '再次點擊確認刪除',
        'Delete Section Confirmation': '確定刪除該標題及其章節內的全部內容嗎？',
        'Duplicate': '複製段落',
        'Cut Block': '剪下段落',
        'New Paragraph': '新增段落',
        'Insert Above': '在上方插入',
        'Insert Below': '在下方插入',
        'Move Up': '上移',
        'Move Down': '下移',
        'Comment': '批註',
        'Delete Selection': '刪除所選文字',
        'Delete': '刪除段落',
        // imageToolbar
        'Edit Image': '編輯圖片',
        'Edit Block': 'Edit Block',
        'Delete Block': 'Delete Block',
        'Inline Image': '行內圖片',
        'Remove Image': '刪除圖片',
        // ImageSelector
        'Select': '選擇',
        'Embed link': '插入連結',
        'Choose Image': '選擇圖片',
        'Choose image from your computer.': '從你的電腦選擇圖片。',
        'Alt text': '替代文字',
        'Image link or local path': '圖片連結或本機路徑',
        'Image title': '圖片標題',
        'Embed Image': '嵌入圖片',
        'Paste web image or local image path. Use': '貼上網路圖片或本機路徑。切換模式',
        'simple mode': '精簡模式',
        'full mode': '完整模式',
        // preview block
        'Loading...': '載入中...',
        'Invalid Diagram Code': '圖表渲染失敗',
        'Empty Diagram': '空圖表',
        'Input Mathematical Formula...': '輸入數學公式...',
        'Input Front Matter...': '輸入頁首資訊...',
        'Invalid Mathematical Formula': '數學公式錯誤',
        'Empty Mathematical Formula': '空數學公式',
    },
};
