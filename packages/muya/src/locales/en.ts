export const en = {
    name: 'en',
    resource: {
    // tableTools
        'Insert Row Above': 'Insert Row Above',
        'Insert Row Below': 'Insert Row Below',
        'Remove Row': 'Remove Row',
        // tableColumnTools
        'Align Left': 'Align Left',
        'Align Center': 'Align Center',
        'Align Right': 'Align Right',
        'Insert Column left': 'Insert Column left',
        'Insert Column right': 'Insert Column right',
        'Remove Column': 'Remove Column',
        'Insert Column Left': 'Insert Column Left',
        'Insert Column Right': 'Insert Column Right',
        'Move Row Up': 'Move Row Up',
        'Move Row Down': 'Move Row Down',
        'Move Column Left': 'Move Column Left',
        'Move Column Right': 'Move Column Right',
        'Reset Column Width': 'Reset Column Width',
        // quickInsert
        'Paragraph': 'Paragraph',
        'Horizontal Line': 'Horizontal Line',
        'Front Matter': 'Front Matter',
        'Heading 1': 'Heading 1',
        'Heading 2': 'Heading 2',
        'Heading 3': 'Heading 3',
        'Heading 4': 'Heading 4',
        'Heading 5': 'Heading 5',
        'Heading 6': 'Heading 6',
        'Set Number': 'Set Number',
        'Continue Heading Numbering': 'Continue Heading Numbering',
        'Restart Numbering': 'Restart Numbering',
        'Set Number Value': 'Set Number Value',
        'Number Value': 'Number Value',
        'Cancel': 'Cancel',
        'Table Block': 'Table Block',
        'Display Math': 'Display Math',
        'HTML Block': 'HTML Block',
        'Code Block': 'Code Block',
        'Quote Block': 'Quote Block',
        'Highlight Block': 'Highlight Block',
        'Emoji': 'Emoji',
        'Order List': 'Order List',
        'Bullet List': 'Bullet List',
        'To-do List': 'To-do List',
        'Vega Chart': 'Vega Chart',
        'Mermaid': 'Mermaid',
        'Plantuml': 'Plantuml',
        'basic blocks': 'basic blocks',
        'headings': 'headings',
        'advanced blocks': 'advanced blocks',
        'list blocks': 'list blocks',
        'diagrams': 'diagrams',
        'No result': 'No result',
        'Search keyword...': 'Search keyword...',
        'Type / to insert...': 'Type / to insert...',
        'Copy anchor link to this heading': 'Copy anchor link to this heading',
        'Click to add an image': 'Click to add an image',
        'Load image failed': 'Load image failed',
        'Edit Block': 'Edit Block',
        'Delete Block': 'Delete Block',
        // formatPicker
        'Emphasize': 'Emphasize',
        'Italic': 'Italic',
        'Underline': 'Underline',
        'Strikethrough': 'Strikethrough',
        'Highlight': 'Highlight',
        'Font Color': 'Font Color',
        'Background Color': 'Background Color',
        'Text and Background Color': 'Text and Background Color',
        'Default Color': 'Default Color',
        'Inline Code': 'Inline Code',
        'Inline Math': 'Inline Math',
        'Link': 'Link',
        'Edit Link': 'Edit Link',
        'Text': 'Text',
        'Link Address': 'Link',
        'Paste or enter link': 'Paste or enter link',
        'Confirm': 'Confirm',
        'Remove Link': 'Remove Link',
        'Link View': 'Link View',
        'Title View': 'Title View',
        'Copy Link': 'Copy Link',
        'Copy Original Web Link': 'Copy Original Web Link',
        'More': 'More',
        'Image': 'Image',
        'Eliminate': 'Eliminate',
        'Comment': 'Comment',
        'Delete Selection': 'Delete Selection',
        // Code block
        'Copy': 'Copy',
        'Copy content': 'Copy content',
        'Wrap': 'Wrap',
        'Wrap lines': 'Wrap lines',
        'Auto Wrap': 'Auto Wrap',
        'Cancel Wrap': 'Cancel Wrap',
        'Change Language': 'Change Language',
        'Search Languages...': 'Search languages...',
        'Plain Text': 'Plain Text',
        'More Languages': 'More Languages',
        'Fewer Languages': 'Fewer Languages',
        'Input Language Identifier...': 'Input Language Identifier...',
        'Diagram View': 'View',
        'Code and Chart': 'Show Code and Chart',
        'Code Only': 'Show Code Only',
        'Chart Only': 'Show Chart Only',
        'Fullscreen': 'Fullscreen',
        'Diagram Background': 'Diagram Background',
        'Copy Diagram': 'Copy Diagram',
        'Download Diagram': 'Download Diagram',
        // emojiPicker
        'Smileys & Emotion': 'Smileys & Emotion',
        'People & Body': 'People & Body',
        'Animals & Nature': 'Animals & Nature',
        'Food & Drink': 'Food & Drink',
        'Travel & Places': 'Travel & Places',
        'Activities': 'Activities',
        'Objects': 'Objects',
        'Symbols': 'Symbols',
        'Flags': 'Flags',
        // frontMenu
        'Copy Plain Text': 'Copy Plain Text',
        'Copy Markdown': 'Copy Markdown',
        'Copy Section': 'Copy Section',
        'Duplicate Section': 'Duplicate Section',
        'Cut Section': 'Cut Section',
        'Promote Section Headings': 'Promote Section Headings',
        'Demote Section Headings': 'Demote Section Headings',
        'Collapse Section': 'Collapse Section',
        'Expand Section': 'Expand Section',
        'Move Section Up': 'Move Section Up',
        'Move Section Down': 'Move Section Down',
        'Delete Section': 'Delete Section',
        'Confirm Delete Section': 'Click Again to Delete Section',
        'Delete Section Confirmation': 'Delete this heading and all content in its section?',
        'Duplicate': 'Duplicate',
        'Cut Block': 'Cut Block',
        'New Paragraph': 'New Paragraph',
        'Insert Above': 'Insert Above',
        'Insert Below': 'Insert Below',
        'Move Up': 'Move Up',
        'Move Down': 'Move Down',
        'Delete': 'Delete',
        // imageToolbar
        'Edit Image': 'Edit Image',
        'Inline Image': 'Inline Image',
        'Remove Image': 'Remove Image',
        // ImageSelector
        'Select': 'Select',
        'Embed link': 'Embed link',
        'Choose Image': 'Choose Image',
        'Choose image from your computer.': 'Choose image from your computer.',
        'Alt text': 'Alt text',
        'Image link or local path': 'Image link or local path',
        'Image title': 'Image title',
        'Embed Image': 'Embed Image',
        'Paste web image or local image path. Use': 'Paste web image or local image path. Use',
        'simple mode': 'simple mode',
        'full mode': 'full mode',
        // preview block
        'Loading...': 'Loading...',
        'Invalid Diagram Code': 'Invalid Diagram Code',
        'Empty Diagram': 'Empty Diagram',
        'Input Mathematical Formula...': 'Input Mathematical Formula...',
        'Input Front Matter...': 'Input Front Matter...',
        'Invalid Mathematical Formula': 'Invalid Mathematical Formula',
        'Empty Mathematical Formula': 'Empty Mathematical Formula',
    },
};
