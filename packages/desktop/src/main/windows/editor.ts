...
import { addProjectRoot, removeProjectRoot } from 'common/workspaceRoots'
...
class EditorWindow extends BaseWindow {
  ...
  createWindow(
    ...
    appMenu.addEditorMenu(win, { sourceCodeModeEnabled: false })
  ...
  /** Add a directory root to the current window workspace. */
  openFolder(pathname: string): void {
    if (!pathname || this.lifecycle === WindowLifecycle.QUITTED) return

    const currentRoots =
      this.lifecycle === WindowLifecycle.READY
        ? this._openedRootDirectories
        : [...this._openedRootDirectories, ...this._directoriesToOpen]
    const result = addProjectRoot(currentRoots, pathname)
    if (!result.added) return
    const root = result.roots[result.roots.length - 1]

    if (this.lifecycle === WindowLifecycle.READY) {
      const { browserWindow } = this
      const { menu: appMenu, preferences } = this._accessor

      preferences.setItems({ lastOpenedFolder: root })
  ...
  /** Detach a directory root without touching files or already-open tabs. */
  removeFolder(pathname: string): void {
    if (!pathname || this.lifecycle === WindowLifecycle.QUITTED) return
    const remaining = removeProjectRoot(this._openedRootDirectories, pathname)
    if (remaining.length === this._openedRootDirectories.length) return

    this._openedRootDirectories = remaining
    const { browserWindow } = this
    const { preferences } = this._accessor
    if (preferences.getItem<string>('lastOpenedFolder') === pathname) {
  ...
  get openedRootDirectories(): string[] {
  ...
  private _restoreAllState(): void {
    ...
    try {
      ...
      const rootDirectories = bufferState.project?.rootDirectories
        ?? (bufferState.project?.rootDirectory ? [bufferState.project.rootDirectory] : [])
