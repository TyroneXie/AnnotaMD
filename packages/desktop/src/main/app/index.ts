      ...
      for (const pathname of args._) {
        ...
        if (info) {
          ...
          if (process.env.NODE_ENV === 'development') {
            log.info(`AnnotaMD dev open path: ${info.path}`)
          }
        }
      }
    }
  ...
  private _openPathList(pathsToOpen: PathInfo[], openFilesInSameWindow: boolean = false): void {
    ...
    for (const window of _windowManager.windows.values()) {
      if (window.type === WindowType.EDITOR) {
        ...
        const { openedRootDirectories } = editor
        for (const openedRootDirectory of openedRootDirectories) {
          if (directorySet.has(openedRootDirectory)) {
            window.bringToFront()
            directorySet.delete(openedRootDirectory)
          }
        }
        for (const pathname of Array.from(fileSet)) {
          if (editor.focusOpenedFile(pathname)) {
    ...
    if (!openFilesInSameWindow && !openFilesInNewWindow) {
      ...
      const openFolderInNewWindow = this._accessor.preferences.getItem<boolean>(
        'openFolderInNewWindow'
      )
      const activeEditorId = _windowManager.getActiveEditorId()
      const activeEditor = activeEditorId !== null
        ? (_windowManager.get(activeEditorId) as EditorWindow | undefined)
        : undefined

      for (const item of directoriesToOpen) {
        const { rootDirectory, fileList } = item
        if (!openFolderInNewWindow && activeEditor && rootDirectory) {
          activeEditor.openFolder(rootDirectory)
          activeEditor.openTabsFromPaths(fileList)
      ...
      // Files follow their own new-window preference. Directories still honor
      // openFolderInNewWindow independently.
