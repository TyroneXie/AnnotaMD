...
import { computed, ref, reactive, watch, onMounted, onBeforeUnmount, nextTick, markRaw } from 'vue'
...
import AnnotaMDDocumentCommentFooter from '@/components/annotamd/DocumentCommentFooter.vue'
import bus from '@/bus'
...
const currentFileComments = computed(() => {
  ...
  return filePath ? (annotaMDCommentsStore.commentsByFile[filePath] ?? []) : []
})
...
const hasDocumentContent = computed(() => (props.markdown ?? '').trim().length > 0)
const documentCommentFooterTargetId = `annotamd-document-comment-footer-${Math.random()
  .toString(36)
  .slice(2)}`
const documentCommentFooterTarget = `#${documentCommentFooterTargetId}`
const documentCommentFooterReady = ref(false)
...
const getAnnotaMDSelection = (changes: MuyaChange): AnnotaMDSelection | null => {
  if (changes.isCollapsed) return null

  const anchorPath = (changes.anchorPath ?? []) as Array<string | number>
  const focusPath = (changes.focusPath ?? anchorPath) as Array<string | number>
  const anchorKey = anchorPath.join('/')
...
class SimpleImageViewer {
  ...
  _init(url: string, background: string) {
    ...
    this.img.style.backgroundColor = background
...
const setImageViewerVisible = (status: boolean) => {
  ...
  document.body.classList.toggle('annotamd-image-viewer-open', status)
  if (status) {
    editor.value?.hideAllFloatTools()
  }
...
const ensureDocumentCommentFooterTarget = (): void => {
  const scrollPage = editor.value?.editor?.scrollPage?.domNode as HTMLElement | undefined
  if (!scrollPage) return

  let target = scrollPage.querySelector<HTMLElement>(`#${documentCommentFooterTargetId}`)
  if (!target) {
    documentCommentFooterReady.value = false
    target = document.createElement('div')
    target.id = documentCommentFooterTargetId
    target.className = 'annotamd-document-comment-mount'
    target.setAttribute('contenteditable', 'false')
    scrollPage.appendChild(target)
    requestAnimationFrame(() => {
      documentCommentFooterReady.value = true
    })
    return
  }
  ...
  // Muya re-renders the scrollPage on setMarkdown/updateState, which empties
  // all children before appending the new content blocks. That can leave our
  // mount target above the document content. Move it to the very end every time
  // the DOM changes, so the document comment footer always sits after the text.
  if (target.nextElementSibling) {
    scrollPage.appendChild(target)
  }
}

const observeDocumentCommentFooterTarget = (): void => {
  const scrollPage = editor.value?.editor?.scrollPage?.domNode as HTMLElement | undefined
  if (!scrollPage) return

  documentCommentFooterObserver?.disconnect()
  ensureDocumentCommentFooterTarget()
  documentCommentFooterObserver = new MutationObserver(() => {
    ensureDocumentCommentFooterTarget()
  })
  documentCommentFooterObserver.observe(scrollPage, { childList: true })
}
...
const queueAnnotaMDCommentHighlights = (): void => {
  ...
  commentHighlightFrame = requestAnimationFrame(() => {
    ...
    refreshAnnotaMDCommentHighlights()
  })
}
...
onMounted(() => {
  ...
  const options: Record<string, unknown> = {
    ...
    clipboardWriteImage: (source: string) => window.electron.clipboard.writeImage(source),
...
onBeforeUnmount(() => {
  ...
  documentCommentFooterObserver?.disconnect()
  documentCommentFooterObserver = null
  documentCommentFooterReady.value = false
  if (commentHighlightFrame != null) cancelAnimationFrame(commentHighlightFrame)
  commentHighlightFrame = null
...
<style>
body.annotamd-image-viewer-open .mu-front-button-wrapper,
body.annotamd-image-viewer-open .mu-float-wrapper {
  display: none !important;
}

body.annotamd-image-viewer-open .annotamd-sticky-table-header {
  display: none !important;
}

.annotamd-sticky-table-header {
  position: fixed;
  z-index: 35;
  display: none;
  overflow: hidden;
...
.annotamd-sticky-table-header .annotamd-sticky-table-copy {
  margin: 0;
  border: 0;
...
.annotamd-sticky-table-header .mu-table-cell {
  box-sizing: border-box;
  padding: 8px 12px;
  color: #1f2329;
  font-size: inherit;
...
.annotamd-sticky-table-header .mu-table-cell:last-child {
  border-right: 0;
}
...
.editor-component .mu-container h1,
.editor-component .mu-container h2,
.editor-component .mu-container h3,
.editor-component .mu-container h4 {
  color: #1f2329;
...
.editor-component .mu-container p,
.editor-component .mu-container li {
  color: #2f3437;
  font-size: inherit;
  line-height: inherit;
}

.editor-component .mu-container table {
  width: 100%;
  max-width: 100%;
  border-collapse: separate;
  border-spacing: 0;
...
.editor-component .mu-container pre {
  border: 1px solid #eceff3;
  border-radius: 10px;
  background: #f7f8fa;
}

.editor-component .mu-container code {
  border-radius: 5px;
  background: #f2f4f7;
  color: #1f2329;
}

.editor-component .mu-container pre.mu-code-block > code.mu-code {
  border-radius: 0;
  background: transparent;
}

.editor-component .mu-container mark {
  border-radius: 3px;
  background: #fff2b8;
  color: #1f2329;
}
...
.editor-component .mu-container svg {
  max-width: 100%;
}

.editor-component .mu-container figure {
  max-width: 100%;
  overflow: visible !important;
}
...
.editor-component .mu-container figure.mu-table::-webkit-scrollbar {
  height: 8px;
}

.editor-component .mu-container figure.mu-table::-webkit-scrollbar-track {
  background: transparent;
}

.editor-component .mu-container figure.mu-table::-webkit-scrollbar-thumb {
...
.editor-component .mu-container .ag-container-preview {
  max-width: 100%;
  overflow: hidden;
}

.editor-component .mu-container figure.mu-diagram-block {
  box-sizing: border-box;
  border: 1px solid #dee0e3;
  border-radius: 8px;
  overflow: visible !important;
...
.editor-component .mu-container figure.mu-diagram-block::-webkit-scrollbar,
.editor-component .mu-container figure.mu-diagram-block .mu-diagram-preview::-webkit-scrollbar {
  display: none;
}

.editor-component pre[data-role='AnnotaMD'],
.editor-component pre[data-role='annotamd'],
.editor-component figure[data-role='AnnotaMD'],
.editor-component figure[data-role='annotamd'] {
  display: none !important;
}

.editor-component .annotamd-document-comment-mount {
  width: 100%;
  margin: 0;
  padding: 0;
  box-sizing: border-box;
