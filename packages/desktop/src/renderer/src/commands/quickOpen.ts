...
class QuickOpenCommand {
  ...
  run = async(): Promise<void> => {
    ...
    if (_folderState.projectTrees.length === 0 && _editorState.tabs.length === 0) {
  ...
  _doSearch = (query: string): QuickOpenSubcommand[] | Promise<QuickOpenSubcommand[]> => {
    this._cancelFn = null
    const { _editorState, _folderState } = this
    const rootPaths = _folderState.projectTrees.map((tree) => tree.pathname)
    ...
    if (tabsAvailable) {
      ...
      for (const tab of _editorState.tabs) {
        ...
        if (
          pathname &&
          re.test(pathname) &&
          !rootPaths.some((rootPath) => window.fileUtils.isChildOfDirectory(rootPath, pathname))
        ) {
  ...
  _getPath = (pathname: string): { title?: string; description: string } => {
    const rootPath = [...this._folderState.projectTrees]
      .sort((a, b) => b.pathname.length - a.pathname.length)
      .find((tree) => window.fileUtils.isChildOfDirectory(tree.pathname, pathname))?.pathname
    if (!rootPath) {
