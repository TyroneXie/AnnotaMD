...
function loadTranslations(language: string): Translations | null {
  ...
  try {
    ...
    // Development locale files change while Vite is running. Always prefer the
    // checked-in source JSON there so an ignored, stale .min.json cannot leave
    // only part of a settings page translated after HMR.
    ...
    if (process.env.NODE_ENV === 'development' || process.env.PERF_TESTING === 'true') {
      ...
      if (!fs.existsSync(localePath)) {
        const previewPath = path.join(process.cwd(), 'static', 'locales', `${language}.min.json`)
        if (fs.existsSync(previewPath)) {
          localePath = previewPath
        }
      }
    }
