...
describe('AI workspace panel', () => {
  it('renders the DBX-style chat shell and post-apply change review', () => {
    ...
    expect(editor).toContain("editor.value.on('annotamd-agent-selection'")
    expect(app).toContain("window.addEventListener('annotamd:open-agent-with-selection'")
    expect(app).toContain("layoutStore.SET_LAYOUT({ rightColumn: 'agent', showSideBar: true })")
