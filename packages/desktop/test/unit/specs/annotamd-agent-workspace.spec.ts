...
describe('AI workspace panel', () => {
  it('renders the DBX-style chat shell and post-apply change review', () => {
    ...
    expect(header).toContain('data-testid="ai-toggle-maximize"')
    ...
    expect(app).toContain("import { Cpu } from '@element-plus/icons-vue'")
    expect(app).not.toContain('<svg viewBox="0 0 24 24" aria-hidden="true">')
    expect(tabs).toContain('import { Plus, Close }')
    expect(tabs).not.toContain('tab-agent-toggle')
    expect(titleBar).toContain('ArrowRight, ChatLineRound, Cpu')
    expect(titleBar).toContain('<el-icon class="title-pane-icon" aria-hidden="true"><Cpu /></el-icon>')
