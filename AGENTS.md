...
- 现象：Agent 读取全部评论后，可能把“处理评论”错解为“修改所有评论关联正文”，连问题类评论也直接改文档。根因：只在 MCP server instructions 或可选 prompt 中写工作流不够靠近当前决策，Agent 调用读取工具后看到的主要是评论数据。推荐做法：`annotamd_read_document`、`annotamd_list_comments` 和 `annotamd_get_comment` 每次都在数据前返回必须执行的逐条意图分类；问题、讨论和征询意见使用 `annotamd_reply_comment` 回复且默认不解决，只有明确修改建议才能使用 `annotamd_apply_comment_edit`，歧义内容先回复追问。修改工具的 description 也应重复这个调用门槛。验证方式：对读取结果格式做回归测试，同时断言问题、修改建议和歧义三类路由说明都存在，且原始评论 payload 仍可完整解析；构建后再检查打包使用的 `bundle/index.mjs`。
