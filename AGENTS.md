...
- 现象：发送 Agent 消息时偶发 `Could not create the Agent document checkpoint: document-content-changed`，即使用户仍停留在同一篇文档。根因：侧栏请求保存的是响应式状态中的正文快照，而 Muya 在 checkpoint 前会同步刷新同一帧的待处理编辑，两份正文可能短暂不一致。推荐做法：初始 checkpoint 由 Muya 原子捕获当前正文，主进程把返回的 `beforeMarkdown` 同时用于 Agent scope、原生文件变更同步和 Diff；若刷新后文档已变脏，则关闭 checkpoint 并要求先保存，不能让 CLI 基于旧磁盘内容继续执行。验证方式：单测同时覆盖“侧栏旧快照但文档已保存”和“发送期间变脏”，Electron E2E 用故意过期的请求快照确认真实 Agent turn 成功且无错误横幅。
