...
- 现象：选中线程长于当前视口时，继续阅读评论会带动共享滚动并让正文锚点离开原位。根因：评论卡片没有自己的临时滚动边界，所有滚轮输入都会落到正文与评论栏共享的外层滚动。推荐做法：只在指针位于选中且展开的超长卡片内时启用局部滚动，按卡片当前可见空间限制高度，标题行吸顶并阻止滚动链；鼠标移出不立即展开卡片，只有滚动正文或评论栏、拖动外层滚动条、切换评论或收起时才退出。验证方式：Electron E2E 断言卡片 `scrollTop` 增加时正文 `scrollTop` 不变、标题保持吸顶，并确认外部滚动会退出局部模式且恢复正文滚动。
