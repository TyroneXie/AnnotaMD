...
- 现象：批注跨越粗体、行内代码、链接或自动换行时，下划线会因各字体盒分别绘制 `text-decoration` 而出现台阶；给特殊节点增加伪元素补线只能修复单一组合。根因：浏览器按行内排版片段分别计算装饰线基线，不存在一个可由 CSS 偏移保证所有字体和内边距始终一致的位置。推荐做法：CSS Highlight 只绘制选区背景；读取 Range 的 `getClientRects()`，按纵向重叠合并同一可视行，再从该行最左到最右绘制一根独立的 2px 几何线，禁止再为特殊行内节点增加补线规则。验证方式：单测覆盖同一行不同高度片段的合并，Electron E2E 同时覆盖跨行内代码内边距和真实换行，断言每个可视行只有一根线，并用包含普通字体、等宽字体和换行的截图目测确认无台阶。
