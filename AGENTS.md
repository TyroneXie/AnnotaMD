...
- 现象：执行 `npm --prefix packages/desktop run test:e2e -- <单文件>` 仍会同时运行整个 `test/e2e` 目录。根因：`test:e2e` 脚本已经固定包含该目录，后附参数只会追加目标。推荐做法：单文件验证直接在 `packages/desktop` 下执行 `./node_modules/.bin/playwright test <单文件> --config test/e2e/playwright.config.ts --workers=1`。验证方式：启动日志应显示 `Running 1 test using 1 worker`。

### 界面验证结论

- 单元测试和类型检查只能证明逻辑与类型约束通过，不能替代 Electron 实际界面验证。
- 现象：Muya 浮层源码已经被 Vite 热更新，当前 Electron 窗口却仍表现为旧逻辑。根因：热更新替换了模块代码，但不会重建编辑器启动时已经实例化的 UI 插件对象。推荐做法：修改 Muya 插件的实例字段、监听或定位行为后，必须刷新窗口或重启开发版再验收；不能用仍在运行的旧实例判断新代码。验证方式：确认 5173 实际返回的新源码后刷新 Electron，再重新创建选区并复测；自动化用例应启动全新 Electron 进程。
- 现象：Agent 后台健康检查时，整个状态入口会周期性从绿色切成灰色再恢复，像连接异常或卡片闪烁。根因：后台刷新复用了首次加载的 `loading` 展示状态。推荐做法：只在首次解析前显示“检查中”，后续刷新保持上一次已确认状态；检查完成后以独立递增信号仅触发状态圆点的一次轻量光晕，卡片、文字和实心状态色保持稳定。验证方式：单测覆盖后台刷新期间 `loading` 不回退和检查信号递增，并在 Electron 中观察连续心跳期间状态入口无整体变色。
- 现象：发送 Agent 消息时偶发 `Could not create the Agent document checkpoint: document-content-changed`，或未修改正文的 CLI 运行结束后报 `candidate-not-canonical`，即使用户仍停留在同一篇文档。根因：侧栏请求保存的是响应式状态中的正文快照，而 Muya 在 checkpoint 前会同步刷新同一帧的待处理编辑；同时 Muya 内存中的规范 Markdown 与磁盘原文可能只存在表格对齐、空行等序列化差异，不能用两者直接判断 CLI 是否改过文件。推荐做法：初始 checkpoint 由 Muya 原子捕获当前正文；主进程分别保存 checkpoint 的规范正文和运行开始时的磁盘原文，前者用于 Agent scope 与 Diff，后者只用于判断原生 CLI 是否写盘；确认发生原生写盘后才把候选交给 Muya 规范化并应用，MCP 修改仍保持严格的规范输入校验。若刷新后文档已变脏，则关闭 checkpoint 并要求先保存，不能让 CLI 基于旧磁盘内容继续执行。验证方式：单测同时覆盖“侧栏旧快照但文档已保存”“发送期间变脏”和“磁盘原文非规范但未发生写盘”，Electron E2E 用故意过期的请求快照与紧凑表格原文确认真实 Agent turn 成功、磁盘内容不变且无错误横幅。
- 现象：Agent 先说明计划、执行多次工具、补充过程说明、再输出结论时，文字和工具可能失去真实先后顺序，或全部挤进一个过程卡。根因：一轮执行只持久化一条 Assistant 消息，或渲染层按消息类型而不是连续事件分组。推荐做法：在首个文本增量到来时才创建 Assistant 消息，并以 `tool-started` 作为消息分段边界；渲染时保留文字节点原位置，只合并相邻的连续工具消息，最终回答独立展示。验证方式：服务单测断言 `Assistant -> Tool -> Assistant -> Tool -> Assistant` 的持久化顺序，Electron E2E 断言形成两个工具组且过程文字、工具组和最终回答按时间线交错排列。
- 现象：Claude Code 最终评论很短，但执行仍可能因累计输出超过 4 MB 被中止。根因：默认使用 `stream-json --verbose`，工具调用和中间消息也进入 stdout，执行层又把完整事件流累计到内存后才解析最终结果。推荐做法：默认 Claude Code 命令使用 `--output-format json` 且不启用 `--verbose`，只接收单个最终结果；用户自定义的输出格式和调试参数保持原样，不由执行层强制改写。上下文自动压缩不等同于压缩 CLI stdout，仍需保留独立输出上限。验证方式：单测同时覆盖默认补 `json`、保留自定义 `stream-json --verbose`，并用长工具链任务确认默认路径不会累计中间事件。
- 现象：Agent 运行中只看到开场计划气泡，工具调用和思考摘要消失；完成后又可能把开场计划误当最终回答。根因：Host 的 `reasoning-delta` 未进入消息存储，且时间线只在首个工具出现后才建立运行状态。推荐做法：每轮开始即持久化独立运行摘要，保留 CLI 明确提供的思考摘要；界面把思考与过程文字作为独立时间线节点，只折叠连续工具组，最终回答独立渲染，失败状态突出且无最终输出时明确提示。验证方式：服务单测覆盖 reasoning、分段回答与耗时元数据，Electron E2E 覆盖事件顺序、工具组折叠、最终回答位置和历史恢复。
- 现象：使用原生 `details/summary` 实现的弹出菜单，打开后点击正文或另一个菜单仍会停留，用户只能再次点击原触发图标关闭。根因：原生 `details` 只负责触发项自身的开关，不提供点击外部关闭。推荐做法：在组件挂载期间用捕获阶段的 `pointerdown` 监听菜单外点击并关闭，卸载时移除监听；关闭时同时复位二次确认等临时状态。验证方式：单测覆盖监听的添加与移除，Electron 中依次验证点击正文和打开相邻菜单都会收起原菜单。
...
- 现象：点击居中图片后，图片工具栏可能贴到窗口左上角。根因：居中图片容器会扩展为整行，而图片选中流程还会重建 DOM，使异步定位持有的图片元素失效。推荐做法：点击时同步保存可见 `img` 的边界矩形，用该虚拟锚点定位工具栏；缩放条仍锚定图片容器。验证方式：Electron E2E 同时比较工具栏与图片的水平中心，并覆盖同源重复图片，不能只断言工具栏可见。
- 现象：图片缩放时可能连续抛出 `querySelector` 或 `style` 的空引用错误，错误框关闭后仍无法继续操作，只能重启。根因：图片移出后仍有排队中的缩放条渲染任务，且拖拽期间隐藏缩放条没有同步移除 document 级 `mousemove`/`mouseup` 监听，后续事件会继续访问已经清空的图片引用和手柄 DOM。推荐做法：延迟渲染必须核对当前引用且在隐藏时取消；缩放条隐藏或销毁时必须同步解绑拖拽监听、清空拖拽状态，所有异步定位入口还要校验引用仍连接且控件 DOM 完整。验证方式：单测覆盖“排队显示后引用清空”和“拖拽中隐藏”两个竞态；Electron E2E 真实拖拽并检查宽度写回 Markdown、渲染错误计数为零，最后在实际界面连续从不同角缩放并继续编辑正文。
- 现象：点击图片后图片工具栏与先前的文字格式工具栏可能同时显示。根因：图片点击会阻止冒泡，旧工具栏收不到 document click；而已经排队的 `requestAnimationFrame` 还可能在被隐藏后重新执行 `show()`。推荐做法：图片工具栏显示前定向关闭格式工具栏；格式工具栏执行延迟显示时若图片工具栏仍在显示，则丢弃这次过期请求。验证方式：Electron E2E 覆盖“文字选区 → 点击图片”和“图片 → 点击正文并重新选字”两个方向，并以浮层包装器的 opacity/坐标判断真实显示状态。
- 现象：文字仍保持选中时滚动正文，格式工具栏会消失、停在滚动前的位置，或在选区离屏后被钉在窗口边缘。根因：通用浮层基类把滚动超过 50px 视为离开操作目标并统一隐藏；`getCursorReference()` 保存的是选中瞬间的静态矩形；Floating UI 的 `flip`、纵向 `shift` 与安全坐标钳制还会强制让浮层留在视口。推荐做法：仅让持续选区工具栏豁免滚动隐藏，使用带选区上下文元素的实时虚拟锚点，并关闭它的纵向翻转、偏移与安全钳制，只保留横向防溢出；普通菜单继续沿用滚动关闭。验证方式：Electron E2E 在选区不变时先滚动超过阈值，断言选区文本和工具栏均保留且相对位置不变；再把选区完全滚出窗口，确认工具栏也随之离屏，滚回后两者一起恢复。
...
- 现象：批注跨越粗体、行内代码、链接或自动换行时，下划线会因各字体盒分别绘制 `text-decoration` 而出现台阶；跨段落和列表时，跨块 Range 还可能返回列表等块级容器的整行矩形，把文字后的空白也画上线。根因：浏览器按行内排版片段分别计算装饰线基线，且跨块 `Range.getClientRects()` 会混入块级容器盒。推荐做法：CSS Highlight 继续使用完整跨块 Range 绘制选区背景；几何线则按每个 `.mu-content` 拆成内容块内 Range 后读取 `getClientRects()`，按纵向重叠合并同一可视行，再从该行最左到最右绘制一根独立的 2px 线，禁止直接使用跨块 Range 的矩形，也禁止为特殊行内节点增加补线规则。验证方式：单测覆盖内容块 Range 拆分和同一行不同高度片段的合并；Electron E2E 同时覆盖跨行内代码内边距、真实换行及“段落 → 列表 → 段落”，断言每个文字块或可视行只有一根线，且线的左右端不得超出对应文字盒。
- 现象：删除线跨越普通文字与行内代码时，即使分别微调 CSS 渐变的百分比位置，线条仍会在字体交界处形成台阶。根因：普通字体、等宽字体和代码内边距使用不同的行内盒，百分比背景仍在各自盒内计算。推荐做法：保留 CSS 渐变作为无脚本回退；编辑器运行时关闭这些背景，读取外层 `del` 的 `getClientRects()`，按每个可视行绘制一根覆盖在代码背景之上的 1px 几何线。验证方式：单测覆盖多行片段坐标与空片段过滤，并在 Electron 中用“普通文字 + 行内代码 + 自动换行”真实文档目测所有字体交界处无台阶。
...
- 现象：Agent 栏打开时点击已有批注的正文会强行切换到批注栏，导致无法选中同一段文字继续向 Agent 提问。根因：正文批注点击处理无条件发起评论聚焦，而评论面板可见状态会驱动右侧模式切换。推荐做法：Agent 模式下只更新活动批注高亮，不发起评论聚焦；右侧关闭或批注模式仍保留原有定位行为。验证方式：Electron E2E 创建批注后切到 Agent，依次点击和选择批注文字，确认 Agent 保持可见、批注栏不出现且选区进入 Agent 上下文。
...
- 现象：短而被内部滚动截断的评论不便阅读，但为所有评论提供“适合阅读”按钮会增加额外概念，并让普通卡片点击频繁带动正文。根因：是否需要放大应由真实溢出和当前可视占比共同决定；只对正文发起平滑滚动还可能被评论栏的双向同步抵消。推荐做法：仅在点击评论卡片本身后，同时满足 `scrollHeight > clientHeight` 且卡片高度小于评论栏可视高度的 `2/3` 时，自动放大到完整内容所需高度与 `2/3` 的较小值；按钮、输入、编辑不触发，达到或超过 `2/3` 的长评论继续内部滚动。放大后若底部溢出，直接把正文与评论栏写到同一 `scrollTop`，只上移到卡片底部可见；切换评论后上一条恢复普通布局。验证方式：Electron QA 分别覆盖“小且截断”“已达 `2/3` 仍截断”和编辑控件点击，记录卡片高度、溢出、底部坐标及两侧滚动位置。
...
- 现象：在已有批注范围内再次划选并点击批注后，新评论输入框已经创建，但视觉上像是没有出现。根因：同锚点布局仍把旧的选中评论固定在正文锚点，新建输入框排在已保存评论之后，被挤到当前视口下方。推荐做法：评论输入框打开时把它作为当前选中气泡，并排在同锚点已保存评论之前；旧线程向下让位。数据库和 MCP 继续按独立 `commentId` 保存、列出和读取重叠评论，不增加锚点唯一约束。验证方式：Electron E2E 先创建一条评论，再用真实鼠标拖选其高亮范围，确认新输入框位于旧卡片上方且在视口内，并能提交为第二条独立评论。
- 现象：选区评论已经输入草稿后再次划选正文，未发送评论的引用和高亮会跟随新选区移动。根因：编辑器持续把每次 `selection-change` 写入共享 `activeSelection`，评论框没有区分空草稿与已开始撰写的草稿。推荐做法：评论框出现非空内容后锁定当前选区，同时暂存后续选区；草稿清空时才切换到暂存选区，取消、提交或换文档时清除锁和暂存状态。验证方式：Electron E2E 覆盖“选中整段 → 输入草稿 → 再选小段/其他正文”，断言引用、下划线和草稿仍属于原选区；清空草稿后引用才切换。
...
- 现象：用户主动点击“检查更新”后，在没有新版本或检查失败时界面没有任何反馈。根因：更新结果只写入共享状态，而侧栏会隐藏“已是最新版”状态。推荐做法：带窗口来源的手动检查使用原生对话框反馈最新版、失败或不支持；启动后的后台检查继续保持静默。验证方式：单测同时覆盖手动检查有反馈和后台检查无弹窗，并用当前正式版本连接真实 Release 检查一次。
- 现象：macOS 临时签名包能够发现并下载新版本，但重启安装时报代码签名校验失败。根因：Squirrel.Mac 要求新旧应用满足可信代码签名要求，ad-hoc 签名不具备可验证的 Team Identifier。推荐做法：运行时检查当前应用签名；只有可信签名包自动下载并安装，ad-hoc 包保留版本检查但明确引导用户从 Releases 手动安装。验证方式：用旧版本包检查并下载当前 Release，核对安装器签名错误日志；同时单测可信签名与 ad-hoc 签名两条分支。
