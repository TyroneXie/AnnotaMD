...
- 现象：选中线程长于当前视口时，继续阅读评论会带动共享滚动；若滚动正文就立即解除局部模式，卡片还会在完整高度与受限高度之间反复伸缩；从评论 A 的中部切换到评论 B，再通过“展开全部”返回 A 时还可能沿用旧位置。根因：评论卡片没有稳定的局部滚动边界，且拦截每次 `wheel` 后手动累加 `scrollTop` 会丢失 macOS 原生滚动惯性；外部滚动触发重置又会改变卡片高度并引发锚点布局重排；展开按钮直接切换选中评论时只尝试重置目标卡片，没有清除上一张卡片仍保存在 DOM 中的 `scrollTop`。推荐做法：只在指针位于选中且展开的超长卡片内时启用局部滚动，首次滚轮按当前可见空间限制高度，后续交给原生 `overflow-y: auto` 并用 `overscroll-behavior` 阻止滚动链；激活后保留高度和阅读位置，滚动正文或评论栏只移动共享锚点，不解除局部模式；切换评论、收起线程、删除评论或换文档时必须同时清除局部模式并把离开的卡片 `scrollTop` 归零。布局计算应立即使用受限高度，避免等待 `ResizeObserver` 再发生第二次重排。验证方式：Electron E2E 断言激活后的 wheel 未被取消、卡片 `scrollTop` 原生增加而正文不动，外部滚动后卡片仍保持局部模式且正文正常滚动；再覆盖“评论 A 滚到中部 → 展开评论 B → 重新展开 A”，确认 A 从顶部开始，并检查导航与共享滚动条回归。
