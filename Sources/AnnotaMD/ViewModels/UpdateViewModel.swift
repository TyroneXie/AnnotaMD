...
final class UpdateViewModel {
    ...
    private let logger = Logger(subsystem: "com.xielintao.annotamd", category: "UpdateViewModel")
        ...
        downloadTask = Task {
            do {
                ...
                let workDir = FileManager.default.temporaryDirectory
                    .appendingPathComponent("AnnotaMD-update-\(UUID().uuidString)")
                ...
                // 验证新 .app 包含可执行文件
                let executableName = Bundle.main.infoDictionary?["CFBundleExecutable"] as? String ?? "AnnotaMD"
    ...
    private func performZIPInstall() async throws {
        ...
        // 可执行文件名（产品名为 AnnotaMD；不可硬编码，否则验证恒失败 → 误回滚到旧版本）
        let executableName = Bundle.main.infoDictionary?["CFBundleExecutable"] as? String ?? "AnnotaMD"
        ...
        let scriptContent = """
        #!/bin/bash
        # AnnotaMD 守夜人脚本：等待旧进程退出 → 原子替换 → 重启
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("AnnotaMD-nightwatch-\(UUID().uuidString).sh")
        ...
        let logURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("AnnotaMD-update-\(UUID().uuidString).log")
