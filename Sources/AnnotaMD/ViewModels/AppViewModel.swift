...
final class AppViewModel {
    ...
    /// 窗口标题
    var windowTitle: String = "AnnotaMD"
    ...
    private func updateWindowTitle() {
        if hasUnsavedUntitled {
            windowTitle = "AnnotaMD — \(untitledFileName)"
        } else if isSingleFileMode, let url = singleFileURL {
            windowTitle = "AnnotaMD — \(url.lastPathComponent)"
        } else if let dir = rootDirectory {
            windowTitle = "AnnotaMD — \(dir.lastPathComponent)"
        } else {
            windowTitle = "AnnotaMD"
        }
    }
}
